...
export const newAddress = asyncErrorHandler(async (req: Request, res, next) => {
  // console.log(req.body)
  const { fullName, mobileNo, houseNo, pinCode, area, city, state, place } =
    req.body;
...
export const updateAddress = asyncErrorHandler(
  async (req: Request, res, next) => {
    ...
    const { fullName, mobileNo, houseNo, pinCode, city, area, state, place } =
      req.body;
    ...
    if (req.user._id != address.user) {
      return next(
        new ErrorHandler("You are not authorized to update this address", 401)
      );
    }
...
export const deleteAddress = asyncErrorHandler(async (req, res, next) => {
  ...
  if (!req.user._id.equals(address.user)) {
    return next(
